...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    ...
    try {
      ...
      // ★ 指数バックオフ: 2^(3-retryAttempts) * 1000ms, 最大5秒
      const waitMs = Math.min(1000 * Math.pow(2, 3 - retryAttempts), 5000)
      console.log(`[リトライ待機] ${waitMs}ms 後に再試行します (残り${retryAttempts}回)`)
      await delay(waitMs)
