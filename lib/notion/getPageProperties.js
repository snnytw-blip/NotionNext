...
export default async function getPageProperties(
  ...
  for (let i = 0; i < rawProperties.length; i++) {
    ...
    const schemaEntry = schema?.[key]
      ...
      // [task3_category_site_filter] site を knownNames に追加
      ...
      if (knownNames.includes(key)) {
        ...
        console.log('[DEBUG MAP FALLBACK knownNames]', key, 'val:', JSON.stringify(getTextContent(val)).substring(0, 50))
      } else if (notionPropNames.includes(key)) {
        // key matches a NOTION_PROPERTY_NAME value → use as-is
        properties[key] = getTextContent(val)
        console.log('[DEBUG MAP FALLBACK notionPropNames]', key, 'val:', JSON.stringify(getTextContent(val)).substring(0, 50))
      } else {
        ...
        for (const [propKey, propName] of Object.entries(BLOG.NOTION_PROPERTY_NAME)) {
          if (propName === key) {
            ...
            console.log('[DEBUG MAP FALLBACK reverse]', propKey, 'val:', JSON.stringify(getTextContent(val)).substring(0, 50))
            break
          }
        }
      }
  ...
  // ★ RAW SCAN FALLBACK: iterate value.properties for any date-type value, regardless of schema
  if (!properties.date) {
    console.log('[DEBUG DATE SCAN]', id, 'starting raw scan for date...')
    for (const [rawKey, rawVal] of Object.entries(value?.properties || {})) {
      const dateVal = getDateValue(rawVal)
      if (dateVal && dateVal.start_date) {
        console.log('[DEBUG DATE SCAN FOUND] key:', rawKey, 'start_date:', dateVal.start_date, 'end_date:', dateVal.end_date)
        delete dateVal.type
    ...
    if (!properties.date) {
      console.log('[DEBUG DATE SCAN]', id, 'no date found in raw scan')
    }
  }
  ...
  // ★ DIAGNOSTIC: properties after field mapping
  console.log('[DEBUG PROPERTIES AFTER MAPPING] id:', id,
    'keys:', Object.keys(properties),
    'date:', JSON.stringify(properties?.date),
    'dateType:', typeof properties?.date,
    'publishDate:', properties?.publishDate,
    'allDateLikeKeys:', Object.keys(properties).filter(k =>
      properties[k] && typeof properties[k] === 'object' && properties[k]?.start_date
    ))
  ...
  // [task3_category_site_filter] site も同様に単一文字列として取得
...
try {
  ...
  //    properties.date がオブジェクトでない場合（配列など）のガードを追加
    ...
    if (properties?.date && typeof properties.date === 'object' && !Array.isArray(properties.date)) {
      ...
      console.log('[DEBUG DATE STEP1] found object date:', rawDate)
    }

    // ★ FIX: Handle string date (from schema-less fallback path, where getTextContent returns "2024-05-02" instead of an object)
    if (!rawDate && properties?.date && typeof properties.date === 'string') {
      rawDate = properties.date
      console.log('[DEBUG DATE STEP1b] found string date:', rawDate)
    }

    // ★ DIAGNOSTIC: fallback search through all properties for start_date
    if (!rawDate) {
      for (const propKey of Object.keys(properties)) {
        const propVal = properties[propKey]
        if (propVal && typeof propVal === 'object' && !Array.isArray(propVal) && propVal.start_date) {
          console.log('[DEBUG DATE FALLBACK] found start_date in property:', propKey, propVal.start_date)
          rawDate = propVal.start_date
          break
        }
      }
    }
    ...
    if (!rawDate) {
      ...
      console.log('[DEBUG DATE STEP2] using created_time:', rawDate)
    }

  // 3. 最終フォールバック: 現在時刻
  if (!rawDate) {
    rawDate = new Date().getTime()
    console.log('[DEBUG DATE STEP3] using current time')
  }

  const testDate = new Date(rawDate)
  if (isNaN(testDate.getTime())) {
    console.log('[DEBUG DATE INVALID] rawDate:', rawDate, 'type:', typeof rawDate, 'rawDateJSON:', JSON.stringify(rawDate))
    properties.publishDate = new Date().getTime()
  } else {
    properties.publishDate = testDate.getTime()
  }
  console.log('[DEBUG DATE RESULT] publishDate:', properties.publishDate, 'dateStr:', new Date(properties.publishDate).toISOString(), 'rawDateUsed:', rawDate)
} catch (e) {
  console.log('[DEBUG DATE EXCEPTION]', e.message)
  properties.publishDate = new Date().getTime()
}
