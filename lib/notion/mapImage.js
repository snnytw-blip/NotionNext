 ...
 * 画像マッピング
 *
 * @param {*} img 画像URL（相対パスまたは外部リンク）
 * @param {*} block データブロック（コンテンツブロックまたはPage）
 * @param {*} type block: 単一コンテンツブロック, collection: コレクションリスト
 * @param {*} from 出典
 * @returns
 */
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  // 変換が必要な URL：AWS 図床のアドレス、またはブックマークタイプの外部リンク画像を識別
  // Notion の新しい図床リソース形式は attachment:${id}:${name}
  const needConvert =
    !hasConverted &&
    (block.type === 'bookmark' ||
  ...
  if (!isEmoji(ret) && ret.indexOf('notion.so/images/page-cover') < 0) {
    if (BLOG.RANDOM_IMAGE_URL) {
      // ランダム画像インターフェースが設定されている場合のみ画像を置換
    ...
    // 画像 URL の最適化：各記事の画像 URL を一意にする
    if (
      ret &&
      ret.length > 4 &&
      !ret.includes('https://www.notion.so/images/')
    ) {
      // 画像インターフェースに一意の識別パラメータを付与し、キャッシュによるランダム結果の重複を防止
...
/**
 * 画像を圧縮する
 * 1. Notion 図床は url-query パラメータを指定して圧縮・トリミングが可能（例: ?xx=xx&width=400）
 * 2. Unsplash 画像は API の q=50 で圧縮品質、width=400 で画像サイズを制御可能
 * @param {*} image
 */
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  try {
    ...
    // 解析に失敗した場合、decodeURIComponent してから再解析を試行
    try {
