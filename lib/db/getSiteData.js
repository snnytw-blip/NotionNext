...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    // fetchInBatches uses notionAPI.getBlocks which may not return properties
  // Use getPage for each block individually to get complete data
  if (blockIdsNeedFetch.length > 0) {
    console.log('[DEBUG] Fetching', blockIdsNeedFetch.length, 'blocks individually via getPage')
