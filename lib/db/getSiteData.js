...
async function convertNotionToSiteData(pageId, from, pageRecordMap) {
  ...
  // [task3_category_site_filter] フィルタ処理で再代入するため let に変更
let collectionData = []
  ...
  collectionData.forEach(function (element) {
    ...
    // [task3_category_site_filter] site フィルタ（NEXT_PUBLIC_SITE_FILTER 環境変数で絞り込み）
...
function getSiteInfo({ collection, block, NOTION_CONFIG }) {
  ...
  const avatarConfig = NOTION_CONFIG?.AVATAR || BLOG.AVATAR || ''
  ...
  // アバター画像決定ロジック：
  // AVATAR 設定が空文字でなければ NotionDB アイコンより優先して使用する
  let icon
  if (avatarConfig) {
    icon = avatarConfig
  } else {
